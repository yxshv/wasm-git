...
sed -e "s/fs.mkdirSync(path, node.mode)/fs.mkdirSync(path, {recursive:true,mode:node.mode})/g" -e "s/fs.writeFileSync(path, '', { mode: node.mode });/fs.writeFileSync(path, '', { mode: node.mode, recursive: true });/g" ./libgit2/examples/lg2.js > ./libgit2/examples/lg2.js.tmp
